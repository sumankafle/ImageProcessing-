...
namespace ip {
	...
	*/
	void fourierTransform(const cv::Mat& image, fourier& freqDomain) {
		...
		cv::Mat image32F, coefficients;
		image.convertTo(image32F, CV_32F);
		cv::dft(image32F, coefficients, cv::DFT_COMPLEX_OUTPUT);

		// Set magnitude and phase
		cv::Mat complex[2];
		cv::split(coefficients, complex);
		cv::cartToPolar(complex[0], complex[1], freqDomain.magnitude, freqDomain.phase);
	}
	...
	*/
	void fourierTransformFilterKernel(const cv::Mat& kernel, cv::Size dstSize, fourier& freqDomain) {
		...
		cv::Mat padded = cv::Mat::zeros(dstSize, CV_32F);
	...
	*/
	void fourierInverse(const fourier& freqDomain, cv::Mat& image) {
		...
		cv::Mat complex32F, realParts, imagParts;
		cv::polarToCart(freqDomain.magnitude, freqDomain.phase, realParts, imagParts);

		vector<cv::Mat> complexVec = { realParts, imagParts };
	...
	*/
	void fourierLogPowerSpectrum(const fourier& freqDomain, cv::Mat& powerSpectrum, bool isReorder) {
	...
	*/
	void fourierReorderPowerSpectrum(cv::Mat& powerSpectrum) {
		...
		int uc = cols / 2;
		int vc = rows / 2;
		cv::Mat src = powerSpectrum.clone();
